...
function startRadioBridgePortCopperToSwitch(rbObj) {
    ...
    currentModalObject = null;
    if (typeof showInfo === 'function') {
        showInfo('Прокладка медного кабеля с радиомоста до коммутатора: кликайте по опорам, затем выберите узел сети с коммутатором, ящик с узлом или отдельный коммутатор на карте.', 'Радиомост');
    }
}
