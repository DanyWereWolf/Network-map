...
function getHelpContentHtml() {
    return '<div class="help-section">' +
        ...
        '<li><kbd>Escape</kbd> — отмена текущей операции (режим размещения объекта, прокладка кабеля).</li>' +
        '<li><kbd>Ctrl+Z</kbd> — отменить последнее действие (удаление, добавление объекта или кабеля).</li>' +
        '<li><kbd>Delete</kbd> — удаление выбранного объекта (в режиме редактирования).</li>' +
        '</ul></div>' +
        '<div class="help-section"><h3>Хранение данных</h3>' +
        '<p>Данные хранятся на сервере (база проекта). Запускайте приложение через сервер (npm run api → http://localhost:3000). Для резервного копирования используйте экспорт и импорт.</p></div>' +
