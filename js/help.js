...
function getHelpContentHtml() {
    return '<div class="help-section">' +
        ...
        '<li><strong>Кабель</strong> — ВОЛС 4/8/16/24 жилы. Начало и конец кабеля: муфта или кросс. Опоры используются как промежуточные точки трассы.</li>' +
        '</ul></div>' +
        ...
        '<h4>Прокладка кабеля</h4><p>Нажмите «Проложить кабель», выберите тип кабеля, затем кликните по начальной точке (муфта/кросс). При необходимости добавьте опоры как промежуточные точки, затем кликните по конечной точке (муфта/кросс).</p>' +
        ...
        '<li><strong>Фильтр карты</strong> — в боковой панели можно скрывать/показывать типы объектов (узлы, кроссы, муфты, опоры). Кабели отображаются только между видимыми точками.</li>' +
        ...
        '<li><strong>История изменений</strong> — поддерживает фильтры по пользователю, типу действия и диапазону дат.</li>' +
        ...
        '<li><kbd>Escape</kbd> — закрытие открытого модального окна и отмена текущей операции (режим размещения объекта, прокладка кабеля).</li>' +
        '</ul></div>' +
        '<div class="help-section"><h3>Хранение данных</h3>' +
        '<p>Данные хранятся на сервере (база проекта). Запускайте приложение через сервер (npm run api → http://localhost:3000). Автоматические резервные копии создаются ежедневно в папке <code>data/backups</code>, хранятся 30 дней.</p></div>' +
