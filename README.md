...
- **Проверка обновлений** — при подключении к GitHub приложение может проверять наличие новой версии по релизам репозитория
...
## Контроль версий и обновления через GitHub
...
2. **Создание репозитория на GitHub**: создайте новый репозиторий (например, `Network-map`) на [github.com](https://github.com), не добавляйте README при создании, если файл уже есть локально.

3. **Подключение и первый push**:
   ```bash
   git remote add origin https://github.com/ВАШ_ЛОГИН/Network-map.git
   git branch -M main
   git push -u origin main
   ```

4. **Включение проверки обновлений в приложении**: в файле `main.js` в начале укажите владельца репозитория:
   ```javascript
   const GITHUB_REPO = { owner: 'ВАШ_ЛОГИН', repo: 'Network-map' };
   ```
   Версия приложения задаётся константой `APP_VERSION` (она должна совпадать с тегами релизов).

### Как выкладывать обновления (релизы)

1. Обновите версию в коде:
   - в `main.js`: `const APP_VERSION = '1.0.1';`
   - в `package.json`: `"version": "1.0.1"`
...
3. Создайте **релиз** на GitHub: Repo → **Releases** → **Create a new release** → укажите тег (например, `v1.0.1`), название и описание → **Publish release**.

После этого пользователи при следующей загрузке приложения (или по кнопке «Проверить обновления» в справке) увидят уведомление о новой версии и ссылку на страницу релизов.
