...
- **Визуализация** — узлы, оптические кроссы, кабельные муфты, опоры связи, OLT, сплиттеры, ONU и кабели на карте
- **Трассировка жил** — путь оптического волокна от узла до узла через муфты и кроссы; вывод жил на OLT и на выходы сплиттера к ONU
- **Редактирование** — добавление и удаление объектов, прокладка кабелей, соединение жил в кроссах и муфтах, подключение жил кросса к узлам и OLT, подключение выходов сплиттера к ONU
- **Кабели** — ВОЛС 4/8/16/24 жилы: от муфты/кросса до муфты/кросса с опорами как промежуточными точками; **GPON**: только между OLT и сплиттером (к ONU подключаются через выходы сплиттера)
- **Кросс** — соединение жил между кабелями, вывод жил на узлы сети и на OLT (выбор жилы и кнопки «Подключить к узлу» / «Подключить к OLT»)
- **Сплиттер** — коэффициент деления 1:4 … 1:64; для каждого выхода можно выбрать ONU (как в кросске — «Подключить к ONU»), линии сплиттер–ONU отображаются на карте
...
- **Фильтр карты** — показ только выбранных типов (узлы, кроссы, муфты, опоры, OLT, сплиттер, ONU); кабели отображаются только между видимыми объектами
...
| Оптический кросс | Соединение жил между кабелями, вывод жил на узлы и на OLT («Подключить к узлу» / «Подключить к OLT») |
| Кабельная муфта | Сращивание жил между кабелями |
| Опора связи | Транзитная точка (воздушные линии) |
| OLT | Головное оборудование GPON; к OLT подключаются жилы из кроссов; кабель GPON — только до сплиттера |
| Сплиттер | Деление сигнала (1:4 … 1:64); кабель GPON — только от OLT; к ONU подключаются выходы сплиттера в баллуне («Подключить к ONU» у каждого выхода) |
| ONU | Абонентское оборудование GPON; подключается только через выходы сплиттера (не кабелем) |
| Кабель | **ВОЛС** 4/8/16/24 жилы: от муфты/кросса до муфты/кросса, опоры — промежуточные точки. **GPON**: только OLT ↔ Сплиттер (одно волокно). |

**Добавление:** в режиме редактирования выберите тип в боковой панели и кликните по карте.

**Прокладка кабеля:** **Проложить кабель** → выберите тип (ВОЛС или GPON) → клик по начальной и конечной точке. Для ВОЛС: муфта/кросс/крепление, при необходимости опоры как промежуточные точки. Для GPON: только OLT и сплиттер (к ONU не прокладывается — подключение через баллун сплиттера).

**Кросс:** в баллуне кросса отображаются жилы кабелей; для свободной жилы доступны «Подключить к узлу» и «Подключить к OLT» (выбор из списка). Линии кросс–узел и кросс–OLT отображаются пунктиром на карте.

**Сплиттер:** в баллуне — список выходов (по коэффициенту деления); у каждого выхода — «Подключить к ONU» или отображение подключённого ONU. Линии сплиттер–ONU отображаются на карте. После переноса OLT, сплиттера или ONU баллун не открывается автоматически — только по явному клику.
...
- **1.0.4** — **GPON:** кабель только OLT ↔ Сплиттер; к ONU подключаются через выходы сплиттера (в баллуне сплиттера — «Подключить к ONU» по каждому выходу). **Кросс:** вывод жил на OLT (как на узлы) — кнопка «Подключить к OLT», выбор OLT из списка, линии кросс–OLT на карте. **Сплиттер:** управление выходами (1:4 … 1:64), привязка каждого выхода к ONU, линии сплиттер–ONU. После переноса OLT/сплиттера/ONU баллун не открывается (только по клику). Легенда и сообщения обновлены под новую схему GPON.
