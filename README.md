...
Все данные (карта, пользователи, история, тема, имена групп) можно хранить на сервере в SQLite.

1. Установите зависимости: `npm install`
2. Запустите API-сервер: `npm run api` (по умолчанию порт 3000). База создаётся в папке `data/network-map.db`. При первом запуске создаётся администратор: `admin` / `admin123`.
3. В файле **js/config.js** укажите адрес API: `var API_BASE = 'http://localhost:3000';` (или адрес сервера в сети).
4. Откройте приложение в браузере. Вход выполняется через API; данные карты загружаются и сохраняются на сервере.

Без указания `API_BASE` приложение работает как раньше — только с localStorage (без сервера).
