...
- **Обновления** — кнопка в шапке открывает окно с текущей версией и проверкой наличия новой версии
- **Синхронизация** — совместная работа нескольких пользователей над одной картой в реальном времени (нужен сервер синхронизации)
- **База данных** — хранение карты, пользователей, истории и настроек в SQLite на сервере (REST API)
...
- **Справка** — кнопка «?» в шапке: подробная инструкция по программе.
- **Обновления** — кнопка со стрелкой вниз в шапке: окно с текущей версией и проверкой наличия новой версии.
...
- **Синхронизация** — в боковой панели блок «Синхронизация»: укажите адрес WebSocket-сервера (например `ws://localhost:8765`), нажмите «Подключиться». Изменения карты будут автоматически отправляться на сервер и рассылаться всем подключённым. Чтобы запустить сервер: `npm install`, затем `npm run sync` (или `node server.js 8765`). Сервер нужно запустить на одном компьютере в сети; остальные подключаются к нему по адресу вида `ws://IP:8765`.
...
Все данные хранятся на сервере в SQLite (карта, пользователи, сессия, история, тема, имена групп). Приложение работает только при запуске через сервер проекта; localStorage не используется.

1. Установите зависимости: `npm install`
2. Запустите сервер: `npm run api` (порт 3000). Сервер отдаёт и приложение, и API — база всегда из проекта.
3. Откройте в браузере **http://localhost:3000**. Токен и сессия хранятся в sessionStorage (до закрытия вкладки).

База создаётся в `data/network-map.db`; при первом запуске создаётся администратор: `admin` / `admin123`. Если открыть приложение с диска (file://) или с другого сервера — отобразится сообщение с просьбой открыть его по адресу сервера (http://localhost:3000).

Для резервного копирования используйте **Экспорт** и **Импорт** в боковой панели.

### Настройка сервера (порт, хост)

Скопируйте `server-config.example.json` в `server-config.json` и при необходимости измените:

| Параметр | По умолчанию | Описание |
|----------|--------------|----------|
| `port`   | 3000         | Порт, на котором слушает сервер |
| `host`   | "0.0.0.0"    | Адрес: `0.0.0.0` — доступ по localhost и по IP в сети; `127.0.0.1` — только с этого компьютера |

Пример для другого порта: `"port": 8080`. Переменные окружения `PORT` и `HOST` переопределяют значения из файла.
...
├── main.js      # Основная логика приложения (карта, объекты, кабели, UI)
├── auth.js      # Логика авторизации
├── js/          # Подключаемые модули
│   ├── config.js        # Версия приложения, репозиторий (менять перед релизом)
│   ├── utils.js         # escapeHtml, getObjectTypeName
│   ├── notifications.js # Уведомления (toast)
│   ├── updates.js       # Проверка обновлений (GitHub)
│   ├── help.js          # Модальное окно справки
│   └── history.js       # История изменений, ActionTypes, logAction
│   └── sync.js          # Синхронизация карты (WebSocket)
├── server.js      # Сервер синхронизации (WebSocket) для совместной работы
├── server-api.js  # REST API и база данных: npm run api
├── server-config.example.json  # Пример настроек (порт, хост); скопируйте в server-config.json
├── database.js    # Хранение данных (JSON в data/store.json)
├── data/          # Папка с store.json (создаётся при первом запуске)
