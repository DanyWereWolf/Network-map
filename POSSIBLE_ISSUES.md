...
- **Резервная копия** — при запуске через `server-api.js` первый бэкап создаётся через 1 минуту, далее раз в сутки в `data/backups/` (хранятся 30 дней). Дополнительно можно копировать `data/store.json` или использовать **Экспорт** в приложении.
